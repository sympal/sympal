...
CREATE TABLE page_translation (id BIGINT, title VARCHAR(255), lang CHAR(2), version BIGINT DEFAULT 0, previous_version BIGINT DEFAULT 0, PRIMARY KEY(id, lang)) ENGINE = INNODB;
CREATE TABLE page (id BIGINT AUTO_INCREMENT, disable_comments TINYINT(1) DEFAULT '0', content_id BIGINT, INDEX content_id_idx (content_id), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE content (id BIGINT AUTO_INCREMENT, site_id BIGINT NOT NULL, content_type_id BIGINT NOT NULL, content_template_id BIGINT, master_menu_item_id BIGINT, last_updated_by BIGINT, created_by BIGINT NOT NULL, locked_by BIGINT, is_published TINYINT(1), date_published DATETIME, custom_path VARCHAR(255), layout VARCHAR(255), slug VARCHAR(255), created_at DATETIME, updated_at DATETIME, version BIGINT DEFAULT 0, previous_version BIGINT DEFAULT 0, INDEX content_type_id_idx (content_type_id), INDEX master_menu_item_id_idx (master_menu_item_id), INDEX last_updated_by_idx (last_updated_by), INDEX created_by_idx (created_by), INDEX locked_by_idx (locked_by), INDEX site_id_idx (site_id), INDEX content_template_id_idx (content_template_id), PRIMARY KEY(id)) ENGINE = INNODB;
...
CREATE TABLE content_template (id BIGINT AUTO_INCREMENT, name VARCHAR(255) NOT NULL, type VARCHAR(255) NOT NULL, content_type_id BIGINT NOT NULL, partial_path VARCHAR(255), component_path VARCHAR(255), body LONGTEXT, version BIGINT DEFAULT 0, previous_version BIGINT DEFAULT 0, INDEX content_type_id_idx (content_type_id), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE content_type (id BIGINT AUTO_INCREMENT, name VARCHAR(255) NOT NULL, description LONGTEXT, label VARCHAR(255) NOT NULL, plugin_name VARCHAR(255), list_path VARCHAR(255), view_path VARCHAR(255), layout VARCHAR(255), slug VARCHAR(255), UNIQUE INDEX sluggable_idx (slug), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE route (id BIGINT AUTO_INCREMENT, name VARCHAR(255) NOT NULL, url VARCHAR(255) NOT NULL, type VARCHAR(255), content_id BIGINT, content_type_id BIGINT, slug VARCHAR(255), UNIQUE INDEX sluggable_idx (slug), INDEX content_id_idx (content_id), INDEX content_type_id_idx (content_type_id), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE site (id BIGINT AUTO_INCREMENT, layout VARCHAR(255), title VARCHAR(255) NOT NULL, description LONGTEXT NOT NULL, slug VARCHAR(255), UNIQUE INDEX sluggable_idx (slug), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE versions (id BIGINT AUTO_INCREMENT, record_id BIGINT NOT NULL, record_type VARCHAR(255) NOT NULL, created_by BIGINT, num_changes BIGINT NOT NULL, version BIGINT NOT NULL, created_at DATETIME, INDEX created_by_idx (created_by), PRIMARY KEY(id)) ENGINE = INNODB;
...
ALTER TABLE content ADD FOREIGN KEY (created_by) REFERENCES user(id) ON DELETE CASCADE;
...
ALTER TABLE versions ADD FOREIGN KEY (created_by) REFERENCES user(id) ON DELETE CASCADE;
ALTER TABLE version_change ADD FOREIGN KEY (version_id) REFERENCES versions(id) ON DELETE CASCADE;
