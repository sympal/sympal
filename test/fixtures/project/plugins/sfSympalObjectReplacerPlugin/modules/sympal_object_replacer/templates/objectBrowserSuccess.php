...
<div id="sympal_objects_container">
  ...
  <div id="content_types">
          ...
          <li>
            ...
            <?php else: ?>
              <?php echo jq_link_to_remote($key, array(
                'url' => url_for('@sympal_objects_select?key='.$key),
    ...
    <?php else: ?>
      <h2><?php echo __('No objects have been defined') ?></h2>
    <?php endif; ?>
  </div>

  <div id="sympal_objects_list">
    <?php if ($selectedKey): ?>
      <h2><?php echo $selectedLabel ?> <?php echo __('Objects') ?></h2>
      <ul>
        <?php if (count($objects)): ?>

          <?php foreach ($objects as $object): ?>
            <li rel="<?php echo $selectedKey.':'.$object->id ?>">
