...
myPanel = new YAHOO.widget.Panel('sympal_edit_panel', {
	underlay:"shadow",
	width:"350px",
	x:<?php echo $sf_user->getAttribute('sympal_editor_x', 50) ?>,
	y:<?php echo $sf_user->getAttribute('sympal_editor_y', 50) ?>,
	close:false,
	visible:true,
	draggable:true} );
...
function overlayToggle()
{
  ...
  if (current == '<?php echo __('Hide') ?>')
  {
    ...
    var url = '<?php echo url_for('@sympal_editor_save_state?state=hidden') ?>';
  } else {
    overlay.show();
    toggle.innerHTML = '<?php echo __('Hide') ?>';

    var url = '<?php echo url_for('@sympal_editor_save_state?state=visible') ?>';
  }
...
myPanel.dd.endDrag = function(e, id) {
	...
	var url = "<?php echo url_for('@sympal_save_editor_position?x=##X##&y=##Y##', 'absolute=true') ?>";
