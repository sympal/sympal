...
$(function()
{
  ...
  $('.sympal_slot_form input, .sympal_slot_form textarea').focus(function() {
...
function sympalInsertIntoCurrentEditor(text)
{
  ...
  {
    currentlyFocusedSympalEditor = $('.sympal_slot_form textarea:first');
  }
...
function sympalSetupDropdownMenu(id, control)
{
  ...
  $(control).click(function() {
    if ($(this).attr('rel'))
    {
      if (!ele.html())
      {
        ele.load($(this).attr('rel'), function() {
            ele.slideToggle('fast');
            sympalCloseAllDropdowns();
            $('#sympal_assets_list li a.insert').draggable({
              revert: 'invalid',
            });
        });
      } else {
