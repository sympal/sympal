...
$.widget('ui.sympalSlot', {
    ...
    editor.bind('ajaxResponseSuccess', function() {
      ...
      $('form', editor).submit(function() {
        ...
        $(this).ajaxSubmit( {
          ...
          {
            $('form_body', $(this)).html(responseText);
      ...
      // initialize any slot-specific functionality if applicable
      formClass = 'sfSympalSlot'+self.element.metadata().type;
      if ($.isFunction(self.element[formClass]))
      {
        self.element[formClass](self.element);
      }
      
      /*
      $('input', this).sympalDroppableInput(function(){
        // callback for when something is dropped
      });
      */
  ...
  initialize: function()
  {
    ...
    $('.sympal_slot_content', this.element).bind('dblclick', function() {
      self.openEditor()
    });
